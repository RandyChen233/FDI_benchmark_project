...
class SensorRangeDeviation(TADMethodEstimator):#time steps
    def __init__(
        self,
...
class LNorm(TADMethodEstimator):#time steps
...
class NNDistance(TADMethodEstimator):#time steps
    ...
    def transform(self, x: np.ndarray) -> np.ndarray:
        ...
        return neighbor_distances.min(axis=1) #test times steps x train time steps


class PCAError(TADMethodEstimator):#time steps x features
