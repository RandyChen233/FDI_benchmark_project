...
if __name__ == "__main__":
    ...
    if idx_get_images == True:
        ...
        p4 = Line2D([0], [0], marker="o", color=cledge, label="detected & isolated", markerfacecolor=clfdied,
                    markersize=10)
        ...
        for k in range(Nint):
            ...
            for node, color in color_map_font.items():
                nx.draw_networkx_labels(Gk, posGk, labels={node: node}, font_color=color, font_weight="bold",
                                        font_size=10)
            nx.draw(Gk, posGk, with_labels=False, edge_color=cledge, width=1, node_color=color_map_node_rev,
                    node_size=200)
            plt.legend(handles=patches_all, labelcolor="#000000", fontsize=8, facecolor=cledge, framealpha=1,
                       loc="center left")
            plt.xlim([-20, 7])
            plt.ylim([-7, 7])
            ...
            nx.draw_networkx_labels(Gc, posGc, labels={1: "C"}, font_color=color_font_c, font_weight="bold",
                                    font_size=18)
            plt.legend(handles=patches_all, labelcolor="#000000", fontsize=8, facecolor=cledge, framealpha=1,
                       loc="lower left")
            plt.xlim([2000, 5000])
