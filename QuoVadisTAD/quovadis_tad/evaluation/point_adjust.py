...
def evaluate(scores, targets, pa=True, interval=10, k=0, verbose=True):
    ...
    raw = [results['best_f1_wo_pa'], results['best_precision_wo_pa'], results['best_recall_wo_pa'], results['auc_wo_pa'], results['auprc_wo_pa']]        
    score_dict= {'': metrics_name, 'without_PA': raw}
    
    if pa:
        if verbose:
            print(f'[INFO]: Computing scores with Point Adjust (PA) method by finding best threshold... this may take a while ...')
        ...
        pa_f1_scores = [metrics.f1_score(targets, pak(scores, targets, th, 0)) for th in ths] 
        ...
        pa_scores = pak(scores, targets, ths[np.argmax(pa_f1_scores)], 0)
        ...
        ## datframe to display               
        with_pa = [results['best_f1_w_pa'], results['best_precision_w_pa'], results['best_recall_w_pa'], results['auc_w_pa'], results['auprc_w_pa']]        
        score_dict= {'': metrics_name, 'with_PA': with_pa, 'without_PA': raw}
        
        
        if k != 0:
            if verbose: 
                print(f'computing PA@k scores')
            pak_f1_scores = [metrics.f1_score(targets, pak(scores, targets, th, k)) for th in ths] 
            ...
            pak_scores = pak(scores, targets, ths[np.argmax(pak_f1_scores)], k)
            results['best_precision_w_pak'] = metrics.precision_score(targets, pak_scores)
            results['best_recall_w_pak'] = metrics.recall_score(targets, pak_scores)
            results['auprc_w_pak'] = metrics.average_precision_score(targets, pak_scores)
            results['auc_w_pak'] = metrics.roc_auc_score(targets, pak_scores)
            results['pak_f1_scores'] = pak_f1_scores
            ## datframe to display            
            with_pak = [results['best_f1_w_pak'], results['best_precision_w_pak'], results['best_recall_w_pak'], results['auc_w_pak'], results['auprc_w_pak']]
            score_dict= {'': metrics_name, 'with_PA': with_pa, 'with_PA@K': with_pak, 'without_PA': raw}
