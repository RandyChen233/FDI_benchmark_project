...
def convert_array_to_dataframe_for_plotting(
	sensor_arr: np.array, label_arr: np.array
) -> pd.DataFrame:
	...
	if sensor_arr.ndim > 1:
		...
		sensor_df = pd.DataFrame(
			data=sensor_arr, columns=[f"sensor_{i+1}" for i in range(ncols)]
		)
	else:
		nrows = len(sensor_arr)
		# create pd.dataframe with sensor data
		sensor_df = pd.DataFrame(data=sensor_arr, columns=["sensor"])
	...
	label_df = pd.DataFrame(
		data={
			"startTime_date_time": pd.to_datetime(
				pd.date_range(datetime.today(), freq="S", periods=nrows)
			).astype(str),
			"label": label_arr,
		}
	)
...
def plot_time_series_with_pandas_dataframe(
		...
		shapes.append(
			{
				...
				"xref": "x1",
				"yref": f"y{i+1}",
				"x0": datetime.strptime(
					high_light_df["startTime_date_time"].min(), dt_format
				),
				"y0": df[y_col_name].min(),
				"x1": datetime.strptime(
					high_light_df["startTime_date_time"].max(), dt_format
				),
	...
	if "adjusted_prediction" in cols_to_plot:
		fig.update_yaxes(
			title_text="adjusted_prediction",
			range=[-0.05, 1.05],
			row=cols_to_plot.index("adjusted_prediction") + 1,
			col=1,
		)
