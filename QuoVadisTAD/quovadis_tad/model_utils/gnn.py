...
def compute_finch_adjacency_matrix(train_array, symmetric=True):
     ...
     orig_dist = metrics.pairwise.pairwise_distances(train_array, train_array, metric='cosine')
...
def adj_standard(train_array, topk=30):
    train_array = train_array.T
    orig_dist = metrics.pairwise.pairwise_distances(train_array, train_array, metric='euclidean')
...
def get_graph_info(array, config, verbose=True, standard_adj=False, topk=30):
    
    if not standard_adj:
        adj_n = compute_finch_adjacency_matrix(array)
        adj_n = adj_n.todense()
    else:
        adj_n = adj_standard(array, topk=topk)
    graph_info_nodes = graph_info(adj_n, verbose=True)
    adj_seq = compute_adj_on_slices(array, seq_len=config["input_sequence_length"], slices=500)
    graph_info_seq = graph_info(adj_seq, verbose=True)
    return graph_info_nodes, graph_info_seq


def get_graph_info_3D(array_3d, config, verbose=True, standard_adj=False, topk=30):
    num_scenarios = array_3d.shape[0]
    graph_info_nodes_list = []
    graph_info_seq_list = []

    for i in range(num_scenarios):
        array = array_3d[i]  # Extract the scenario (2D array)
        graph_info_nodes, graph_info_seq = get_graph_info(
            array=array,
            config=config,
            verbose=verbose,
            standard_adj=standard_adj,
            topk=topk
        )
        graph_info_nodes_list.append(graph_info_nodes)
        graph_info_seq_list.append(graph_info_seq)

    return graph_info_nodes_list, graph_info_seq_list
...
class LSTMGC(layers.Layer):
    ...
    def call(self, inputs):
        ...
        if self.return_normal:
            output = tf.transpose(output, [1, 2, 0])  # returns Tensor of shape (batch_size, output_seq_len, num_nodes)
        else:
            output = tf.reshape(output, (batch_size, self.output_seq_len * num_nodes))# returns 2D Tensor of shape (batch_size, output_seq_len x num_nodes]
